// order-form-v6.js — v6's own copy of the order form.
//
// order-form.js is shared by CheckinPallets_23 and _24, both in active use, so
// v6 takes a copy rather than editing it. The original is untouched and keeps
// serving those two; DIST_LOCATION_MAP still lives there and still works for
// them. v6 simply stops consulting it (see deriveLocationFromDist below).
//
// Data access goes through the same _getDb() facade, which under v6 is a view
// over the split tables: db.orderLog and db.orderSchedule are projections of
// the one orders table selected by status, and db.copiedItems is per-browser
// localStorage that is never synced.
...
// v6: the schedule carries a real siteCode, so the location comes from the data
// instead of from DIST_LOCATION_MAP — a six-entry map keyed on the distribution's
// display NAME, which the v3 "Location Day" rename left covering only three of
// fourteen distributions. The map is still consulted as a fallback for any
// pre-v6 caller; it is unchanged and still serves v23/v24 through order-form.js.
function deriveLocationFromDist(dist) {
    if (dist && dist.siteCode) {
        const sites = (_getDb().locationsList || []);
        const site = sites.find(s => s.code === dist.siteCode);
        if (site) return locationValue(site);
        return `${dist.siteCode} - ${dist.siteCode}`.replace(/^(.*) - \1$/, '$1');
    }
